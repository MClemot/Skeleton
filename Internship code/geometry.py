...
def uniform_resampling(isopoints, steps, K, alpha, sigma):
    for step in range(steps):
        ...
        #we compute the distance between the points and the (K+1) nearest neighbors
        dismat = torch.cdist(isopoints, isopoints)
        _,knn = torch.topk(dismat, K+1, dim=1, largest=False)
    
        #we delete the nearest neightbor which is always the point itself, to keep only the K true nearest neighbors
        knn = knn[:,1:]
...
def GD(net, iterations, number, direc):
    points = torch.rand((number, 3), device = device)*1.9-.95
    points[:,2] = .4*points[:,2]-0.1
    points.requires_grad = True
    
    for it in range(iterations):
        points_sdf = net(points.float())
        grad = gradient(points_sdf, points)
        points = points - direc*.01 * grad
        points = points.float()
